...
def create_app():
    ...
    with app.app_context():
        try:
            ...
            # Only build models if we have data
            ...
            if user_count > 0 and book_count > 0:
                ...
                logger.info(f"Insufficient data for models (users: {user_count}, books: {book_count})")
    ...
    def books_search_page():
        ...
        print(f"[DEBUG] details_id from query: {details_id}")
        results = []
        parsed = None
        ...
        if query and not details_id:
            print(f"[DEBUG] is_nl={is_nl} for query: {query}")
            if is_nl:
                parsed = parse_nl_query_with_llm(query)
                print(f"[DEBUG] NLP parsed: {parsed}")
                    ...
                    try:
                        ...
                        for book in filtered_books:
                            if book.plot_embedding:
                                book_embeddings.append(np.array(book.plot_embedding))
                                book_objs.append(book)
                        if book_embeddings:
                            book_embeddings = np.vstack(book_embeddings)
                            from sklearn.metrics.pairwise import cosine_similarity
                            sims = cosine_similarity([query_embedding], book_embeddings)[0]
                ...
                # Standard search logic (title/author)
        ...
        if details_id:
            ...
            print(f"[DEBUG] details_id: {details_id}, book found: {book is not None}")
            if book:
                ...
                if not book_dict.get('genre') or book_dict['genre'] in ('N/A', 'Unknown', None):
                    ...
                    print(f"Trying BookGenre fallback for ISBN: {isbn}")
                    bg = BookGenre.query.get(isbn)
                    print(f"BookGenre lookup result: {bg.genre if bg else 'None'}")
                    if bg:
                ...
                print(f"[DEBUG] details_book.id: {details_book['id'] if details_book else None}")
                print(f"[DEBUG] details_book full: {details_book}")
    ...
    def nl_recommend():
        ...
        recommendations = []
        if comparison and len(filtered_books) > 0:
            # Use sentence-transformers to embed the comparison text
            try:
                query_embedding = rec_system.embedder.encode(comparison)
            ...
            # No comparison, just return top N filtered books
            for b in filtered_books[:10]:
                recommendations.append({
                    'book_id': b.id,
                    'title': b.title,
            ...
            # Fallback: semantic search over all books if no genre/mood match
            try:
                all_books = Book.query.all()
                book_embeddings = []
                book_objs = []
