...
class DatabaseRecommendationSystem:
    ...
    def recommend_content_based(self, user_id: str = None, book_id: str = None, top_n: int = 10) -> List[Dict]:
        ...
        try:
            if book_id:
                ...
                # Get book IDs and details
                all_books = Book.query.all()
                recommendations = []
                
                for i, idx in enumerate(indices[0][1:]):  # Skip the input book
                ...
                # Get average embedding of user's liked books
                user_embeddings = []
                for rating, book in user_ratings:
                    if book.plot_embedding:
                        user_embeddings.append(np.array(book.plot_embedding))
                
                if not user_embeddings:
                    return self._get_popular_recommendations(top_n)
                
                user_profile_embedding = np.mean(user_embeddings, axis=0).reshape(1, -1)
                
                # Find books similar to user profile
                distances, indices = self.embedding_nn.kneighbors(
